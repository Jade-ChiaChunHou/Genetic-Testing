...
cycle1_dic = '../sample_data/' + G2_num +'c-cycle 1.xls'
cycle35_dic = '../sample_data/' + G2_num + 'c-cycle 35.xls'
...
nutrient_gene = pd.read_csv('../raw/nutrient_gene.csv', encoding = 'utf-8')
...
# clinic
if member[(member[['檢體編號']] == G2_num).all(1)].iloc[0,5] != '':
    clinic = member[(member[['檢體編號']] == G2_num).all(1)].iloc[0,5]

elif member[(member[['檢體編號']] == G2_num).all(1)].iloc[0,5] == '':
    clinic = "健樂士診所"
...
print('(7) ' + G2_num + ' finish calculate disease ratio.' + '\n')
...
if gender == "女":
    ...
    print('(8) ' + G2_num + ' finish output ' + fullname + '\n')
    ...
    for rs in nutrient_gene['rs number']:
        ...
        if rs in list(gene_risk['rs_number']):
            ...
            genotype.append(gene_risk.iloc[a, 4])

        elif rs not in list(gene_risk['rs_number']):

            genotype.append(0)

    female_nutrient_risk = pd.DataFrame(list(zip(nutrient_gene['disease'], nutrient_gene['gene'], nutrient_gene['rs number'], nutrient_gene['nutrient'], female_ratio, genotype)), columns=['disease', 'gene', 'rs number', 'nutrient', 'disease ratio', 'gene risk'])
    female_nutrient_risk
    ...
    female_nutrient_result['nutrient'].value_counts()


    # match the nutrient with it own discription
    ...
    female_nutrient_description = pd.DataFrame(list(zip(nutrient_name, description)), columns = ['nutrient name', 'description'])
    ...
    female_disease = ['肥胖症', '第二型糖尿病', '高血压', '高血脂', '高尿酸血症',
                      ...
                      '晚发型阿兹海默症', '帕金森氏症', '退化性关节炎', '骨质疏鬆', '肌少症',
                      '子宫内膜异位', '子宫肌瘤', '多囊性卵巢症候群', '女性尿道感染',
                      '类风湿性关节炎', '红斑性狼疮', '自体免疫甲状腺疾病', '僵直性嵴椎炎',
    ...
    male_nutrient_risk = pd.DataFrame(list(zip(nutrient_gene['disease'], nutrient_gene['gene'], nutrient_gene['rs number'], nutrient_gene['nutrient'], male_ratio, genotype)), columns=['disease', 'gene', 'rs number', 'nutrient', 'disease ratio', 'gene risk'])
    male_nutrient_risk
    ...
    male_nutrient_result['nutrient'].value_counts()
    ...
    male_nutrient_description = pd.DataFrame(list(zip(nutrient_name, description)), columns = ['nutrient name', 'description'])
...
print('Congratulation! '+ G2_num + 'finish all the calculation process from step (1) to (13)!!\n')
